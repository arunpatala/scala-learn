...
object Main extends App{
	import Util._
	val map = Set("TTT","TTH","THT","THH","HTT","HTH","HHT", "HHH").map(x=> x-> 0).toMap
	read22.map{ case(i,s) =>
		val smap = map ++ s.sliding(3).toList.groupBy(x=>x)
		  .mapValues(_.size) 
		  
		  i+" "+smap.toSeq.sortBy(_._1)
		  .reverse.map{_._2}.mkString(" ")
		 }.printn
}

object Util{

	def read22 = readTail.grouped(2).map{_.tuple2}
