...
object Main extends App{
	import Util._
	val l = read17
	val edges = l.flatMap{case(u,v)=>List((u,v),(v,u))}
			.groupBy(_._1).mapValues(_.map(_._2))
	var max = 0
	def length(i:Int, p:Int):Int = {
		val e = edges(i).filter(_!=p)
		if(e.size==0) 0
		else {
			val el = e.map(length(_,i))
			if(el.size>1) max = math.max(max,2+el.sorted.reverse.take(2).sum)
			1+el.max
		}
	}
	max=math.max(max,length(1,-1))
	//edges.toSeq.printn
	println(max)
}
