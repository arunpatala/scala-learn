...
object Main extends App{
	import Util._
	def break(n:Int):Int = n.toString.map(_.toString.toInt).map(x=>x*x).sum
	@tailrec def nums(n:Int,acc:Int):Int = if(break(n)==1) acc else  nums(break(n),acc+1)
