...
object Main extends App{
	...
	def meth(left:List[Int],right:List[Int],sum:Int) = if(right.size==0) sum
	else {
		if(right.head+sum<M) meth(left:+right.head,right.tail,sum)
		else meth(left.tail,right,sum-left.head)
	}

	}

	read.grouped(2).map{ l =>
		val (N,M) = l(0).sInt.tuple2
		val lst = l(1).sInt
