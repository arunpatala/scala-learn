...
object Main extends App{
	import Util._
	@tailrec def read(reader:Iterator[String]) = {
		val (e,f) = reader.next.sInt.tuple2
		val k = reader.next.toInt
		val PW = reader.take(k).sInt.tuple2.toList
		println(meth(f-e,PW))
		read(reader)
	}

	def meth(weight:Int,k:Int, pw:List[(Int,Int)]) = {
		val Z = Array.fromDim[Int](weight,k)	
		for(i<-0 to weight;for ki<-1 to k)
			Z(i)(j) = pw(ki)._2 + Z(i-pw(ki)._1)(ki) , Z(i)(j-1)
