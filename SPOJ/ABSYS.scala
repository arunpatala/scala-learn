...
object Main extends App{
	import Util._
	readTail.map{ s =>
		val l = s.split("[ +=]").filter(_!="")
		val m = l.map(_.contains("machula"))
		if(m(0)) (l(2).toInt-l(1).toInt) + " + " + l(1).toInt + " = " + l(2).toInt
		else if(m(1)) l(0) +" + "+ (l(2).toInt-l(0).toInt) +" = "+ l(2).toInt
		else l(0) +" + "+ l(1) +" = "+ (l(1).toInt+l(0).toInt)
	}.printn
}
