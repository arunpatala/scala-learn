...
object Main extends App{
	import Util._
	val N = 1000000
	val mem = (1 to N).foldLeft(Vector[Long](0)){(v,i)=>
		v:+max(i.toLong,v(i/2)+v(i/3)+v(i/4))
	}
	def conv(i:Long):Long = if(i<=N) mem(i)
				else max(i.toLong,conv(i/2)+conv(i/3)+conv(i/4))

	read.toInt.map{conv}.printn
}
