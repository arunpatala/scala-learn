...
object Main extends App{
	val h = 0;
	read21.map{_.sInt.toList}.map{ t =>
		t.tail.foldLeft(t.head){ case(lprev,lcurr) =>
			val lp = (h::lprev ++ List(h)).sliding(2).toList
			lcurr.zip(lp).map{ case((x,y),z) => math.max(x+z,y+z) }
		}.max
	}.printm
