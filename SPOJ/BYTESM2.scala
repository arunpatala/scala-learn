...
object Main extends App{
	import Util._
	read21.map(_.map(_.sInt.toList)).map{ l =>

		l.tail.foldLeft(l.head){case(lprev,li) =>
			
			val lout = for(i <- 0 until li.size)
				yield	li(i) + {
					if(i==0) max(lprev(i),lprev(i+1))
					else if(i==li.size-1) max(lprev(i-1),lprev(i))
					else max(lprev(i-1),max(lprev(i),lprev(i+1)))
				}
			lout.toList
