...
object Main extends App{
	...
	read21.map{ _.map{_.sInt.toList.tuple2}}.map{
		edges =>
		val bedges = edges ++ edges.map{case(x,y)=>(y,x)}
		val ledges = bedges.groupBy(_._1).map{_.map(_._2)}
		val cmap = mutable.Map[Int,Int]()
		def visit(i:Int,p:Int):Boolean = {
			val check = ledges(i).filter(cmap.contains).map{cmap(_)==cmap(p)}.and
			if(check){
				cmap(i) = 1- cmap(p)
				ledges(i).filter(_!=p).filter(!cmap.contains).foreach(visit(_,i)).and
			}
			else false
		}
...
object Util{
	...
	def readIter(read:Iterator[String]):Stream[List[String]] = if(read.isEmpty) Stream.Empty
								...
								read.take(m).toList #:: readIter(read)
						}
