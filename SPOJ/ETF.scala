...
object Main extends App{
	import Util._
	def fact(i:Int):List[Int] = (2 to math.sqrt(i).toInt).find(i%_==0) match {
		case Some(j) => j :: fact(i/j)
		case None => List(i)
	}
	def etf(n:Int) = fact(n).groupBy(x=>x).mapValues(_.size).map{ x => (x-1,x) }.foldLeft(n){
			case(x1,x) => (n*x1)/x
	}
}
