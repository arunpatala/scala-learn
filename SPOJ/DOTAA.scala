...
object Main extends App{
	import Util._
	@tailrec def read(reader:Iterator[String]) = if(reader.hasNext){
		val l = reader.next.sInt
		val d = l(2)
		val m = l(1)
		val lst = reader.take(l(0)).toInt
		val valid = lst.map{_.toDouble/d}.map{_.ceil}.sum>m
		read(reader)
	}

	readTail.grouped(2).map{_.tuple2}.map{ case (l1,l2) =>
		val (N,K) = l1.tuple2
		val h = l2.sInt.sorted
		val diff = for(i<-0 until N-K)
			yield h(i+K)-h(i)
		diff.min
	}
