...
object Main extends App{
	import Util._
	val l = readTail.toList
	val map = l.groupBy(x=>x).mapValues(_.size)
	//combine 3/4 and 1/4
	val p34 = map.getOrElse("3/4",0)
	val p14 = map.getOrElse("1/4",0)
	val map1 = map ++ if(p34>=p14) Map("3/4"->p34-p14,"1/4"->0)
			else  Map("3/4"->0,"1/4"->p14-p34)
	val q12 = map1.getOrElse("1/2",0)
	val q14 = map1.getOrElse("1/4",0)
	val map2 = map1 ++ if(q12>=q14/2) Map("1/2"->p12-p14/2,"1/4"->p14%2)
			else  Map("1/2"->0,"1/4"->q14-2*q12)
