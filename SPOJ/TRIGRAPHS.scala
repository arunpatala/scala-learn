...
object Main extends App{
	import Util._
	read0.map{_.sInt}.map{ l =>
		val head = l.head
		val cost = List(Inf,head(1),head(1)+head(2))
		l.tail.foldLeft(cost){case(cost,curr) =>
			List(curr(0) + min(cost(0),cost(1)),
			curr(1) + min(cost(0),cost(1),cost(2)),
			curr(2) + min(cost(1),cost(2)))
		}(2)
	}.printn
